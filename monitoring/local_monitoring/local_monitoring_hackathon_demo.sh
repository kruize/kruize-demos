...
function kruize_local() {
	...
	export NAMESPACE="openshift-etcd-operator"
	...
	echo
	echo "######################################################"
	echo "#     Display metadata for openshift-etcd-operator namespace"
	echo "######################################################"
	echo
	...
	echo "######################################################"
	echo
	echo "curl -X DELETE http://${KRUIZE_URL}/createExperiment -d @./create_etcd-operator_expA.json"
	curl -X DELETE http://${KRUIZE_URL}/createExperiment -d @./create_etcd-operator_expA.json
	echo

	echo
	echo "######################################################"
	echo "#     Install metric profile - resource-optimisation-local-monitoring"
	echo "######################################################"
	echo
	curl -X POST http://${KRUIZE_URL}/createMetricProfile -d @./resource_optimisation_local_monitoring.json
	echo

	echo
	echo "######################################################"
	echo "#     Create kruize experiment - openshift-etcd-operator_expA"
	echo "######################################################"
	echo
	echo "curl -X POST http://${KRUIZE_URL}/createExperiment -d @./create_etcd-operator_expA.json"
	curl -X POST http://${KRUIZE_URL}/createExperiment -d @./create_etcd-operator_expA.json
	echo
  	...
  	echo "######################################################"
  	echo
  	echo "curl -X DELETE http://${KRUIZE_URL}/createExperiment -d @./create_etcd-operator_expB.json"
  	curl -X DELETE http://${KRUIZE_URL}/createExperiment -d @./create_etcd-operator_expB.json
  	echo

    echo
    	echo "##########################################################################"
    	echo "#     Install metric profile - resource-optimization-openshift-demo"
    	echo "##########################################################################"
    	echo
    	curl -X POST http://${KRUIZE_URL}/createMetricProfile -d @./resource_optimization_openshift_demo.json
    	echo

  	echo
  	echo "#################################################################"
  	echo "#     Create kruize experiment - openshift-etcd-operator_expB"
  	echo "#################################################################"
  	echo
  	echo "curl -X POST http://${KRUIZE_URL}/createExperiment -d @./create_etcd-operator_expB.json"
  	curl -X POST http://${KRUIZE_URL}/createExperiment -d @./create_etcd-operator_expB.json
  	echo
    ...
    echo
    echo "######################################################"
    echo "#     Calculate the time stamp"
    echo "######################################################"
    echo
    current_time=$(date -u +"%Y-%m-%dT%H:%M:%S")
    formatted_time="${current_time}.000Z"
    echo "$formatted_time"
  	...
  	echo "######################################################"
  	echo
  	curl -X POST "http://${KRUIZE_URL}/generateRecommendations?experiment_name=openshift-etcd-operator_expA&interval_end_time=${formatted_time}"
  	curl -X POST "http://${KRUIZE_URL}/generateRecommendations?experiment_name=openshift-etcd-operator_expB&interval_end_time=${formatted_time}"
  	echo ""
	...
	echo "curl -X POST \"https://${KRUIZE_URL}/generateRecommendations?experiment_name=openshift-etcd-operator_expA&interval_end_time=${formatted_time}\""
	echo "curl -X POST \"https://${KRUIZE_URL}/generateRecommendations?experiment_name=openshift-etcd-operator_expB&interval_end_time=${formatted_time}\""
  	echo
  	echo "List Recommendations using "
	echo "curl http://${KRUIZE_URL}/listRecommendations?experiment_name=openshift-etcd-operator_expA&monitoring_end_time=${formatted_time}"
	echo "curl http://${KRUIZE_URL}/listRecommendations?experiment_name=openshift-etcd-operator_expB&monitoring_end_time=${formatted_time}"
  	echo
  	echo "######################################################"
  	echo
}
...
function get_urls() {
	...
	if [ ${CLUSTER_TYPE} == "minikube" ]; then
		...
		#increase time-out for empty reply
		${kubectl_cmd} annotate route kruize --overwrite haproxy.router.openshift.io/timeout=180s
