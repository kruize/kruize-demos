...
function kruize_local() {
	...
	echo "######################################################"
	echo
	echo "curl http://${KRUIZE_URL}/crawler?org_id=${org_id}&cluster_id=${cluster_id}&timestamp=${timestamp}"
	curl http://${KRUIZE_URL}/crawler?org_id=${org_id}&cluster_id=${cluster_id}&timestamp=${timestamp}"
	echo

	echo "Sleeping for 60s before listing the recommendations!"
	sleep 60s 
